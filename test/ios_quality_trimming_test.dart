...
void main() {
  group('iOS Audio Quality and Trimming Tests', () {
    ...
    testWidgets('iOS high quality conversion uses correct preset', (WidgetTester tester) async {
    ...
    testWidgets('iOS trimming validates time ranges correctly', (WidgetTester tester) async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.setMockMethodCallHandler(channel, (
        MethodCall methodCall,
      ) async {
    ...
    testWidgets('iOS handles precision trimming with microsecond accuracy', (WidgetTester tester) async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.setMockMethodCallHandler(channel, (
        MethodCall methodCall,
      ) async {
        if (methodCall.method == 'trimAudio') {
    ...
    testWidgets('iOS error handling provides detailed error information', (WidgetTester tester) async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.setMockMethodCallHandler(channel, (
        MethodCall methodCall,
      ) async {
        if (methodCall.method == 'convertAudio') {
          throw PlatformException(
            ...
            details: 'AVAssetExportSession failed with detailed error information',
          );
        }
        return null;
      });
      ...
      expect(
        ...
        throwsA(
          isA<PlatformException>()
              ...
              .having((e) => e.message, 'message', contains('Export failed with status'))
              .having((e) => e.details, 'details', contains('AVAssetExportSession failed')),
        ),
      );
    });

    testWidgets('iOS lossless trimming preserves quality', (WidgetTester tester) async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.setMockMethodCallHandler(channel, (
        MethodCall methodCall,
      ) async {
        if (methodCall.method == 'trimAudio') {
    ...
    testWidgets('iOS handles various audio formats correctly', (WidgetTester tester) async {
      final formats = [AudioFormat.m4a]; // Only test M4A as the recommended format
    ...
    testWidgets('iOS metadata preservation works correctly', (WidgetTester tester) async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.setMockMethodCallHandler(channel, (
        MethodCall methodCall,
      ) async {
        if (methodCall.method == 'convertAudio') {
          ...
          return {
            ...
            'bitRate': args['bitRate'],
            'sampleRate': args['sampleRate'],
            'metadata': {'title': 'Test Audio', 'artist': 'Test Artist', 'encoder': 'Flutter Audio Toolkit iOS'},
          };
        }
        return null;
      });
